...
# # Create Nicotine+ directories with correct permissions (user gwb)
...
RUN echo '#!/bin/bash' > /usr/local/bin/start-xpra-custom && \
    ...
    echo '' >> /usr/local/bin/start-xpra-custom && \
    echo 'export XDG_RUNTIME_DIR="/home/gwb/.xdg"' >> /usr/local/bin/start-xpra-custom && \
    ...
    echo '' >> /usr/local/bin/start-xpra-custom && \
    echo 'mkdir -p /home/gwb/.xpra' >> /usr/local/bin/start-xpra-custom && \
    echo 'chmod 700 /home/gwb/.xpra' >> /usr/local/bin/start-xpra-custom && \
    echo '' >> /usr/local/bin/start-xpra-custom && \
    ...
    echo '  --socket-dir="/home/gwb/.xpra" \' >> /usr/local/bin/start-xpra-custom && \
